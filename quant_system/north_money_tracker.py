...
class NorthMoneyTracker:
    ...
    def analyze_flow_trend(self, days: int = 5) -> Dict:
        ...
        if len(dates) < 2:
            return {"trend": "UNKNOWN", "interpretation": "数据不足"}

        net_flows = [self.history["daily"][d].get("north_net_inflow", 0) for d in dates]

        # 计算趋势
        recent_avg = sum(net_flows[: len(net_flows) // 2 + 1]) / max(
            len(net_flows) // 2 + 1, 1
        )
    ...
    def identify_bottom_fishing_signal(
        self,
        stock_code: str,
        north_net_inflow_pct: float,
        price_change: float,
        historical_avg_net: float,
    ) -> Optional[Dict]:
        """
        识别外资抄底信号
        ...
        if north_net_inflow_pct > historical_avg_net * 2 and north_net_inflow_pct > 5:
            return {
                ...
                "action": "买入",
                "confidence": "HIGH"
                if north_net_inflow_pct > historical_avg_net * 3
                else "MEDIUM",
    ...
    def identify_top_escape_signal(
        self,
        stock_code: str,
        ...
        # 外资逆势卖出信号
        if (
            north_net_outflow_pct > abs(historical_avg_net) * 2
            and north_net_outflow_pct > 5
        ):
            return {
                "code": stock_code,
                "signal": "TOP_ESCAPE",
                "action": "卖出",
                "confidence": "HIGH"
                if north_net_outflow_pct > abs(historical_avg_net) * 3
                else "MEDIUM",
    ...
    def generate_north_money_report(self, date: Optional[str] = None) -> str:
        """
        生成北向资金报告
