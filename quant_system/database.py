...
class DatabaseManager:
    ...
    def __init__(
        ...
        if db_type == 'sqlite':
            ...
            if not database_url:
                raise QuantException("PostgreSQL 需要 database_url 参数", config_key='DATABASE_URL')
    ...
    def _init_postgresql(self) -> None:
        ...
        try:
            ...
            raise QuantException("PostgreSQL 需要安装 psycopg2-binary", code="MISSING_DEP")
    ...
    def _create_tables(self):
        ...
        with self.get_cursor() as cursor:
            ...
            # 创建索引
            cursor.execute("CREATE INDEX IF NOT EXISTS idx_trade_code ON trade_logs(code)")
            cursor.execute("CREATE INDEX IF NOT EXISTS idx_trade_timestamp ON trade_logs(timestamp)")
            cursor.execute("CREATE INDEX IF NOT EXISTS idx_rec_code ON recommendations(code)")
    ...
    # ============ 交易日志操作 ============
    def add_trade_log(self, action: str, code: str, price: float, shares: int,
                      reason: str = "", profit_loss: float = 0) -> int:
        """添加交易日志"""
        with self.get_cursor() as cursor:
            cursor.execute("""
    ...
    def get_trade_logs(self, code: Optional[str] = None, limit: int = 100) -> List[Dict]:
        """获取交易日志"""
        with self.get_cursor() as cursor:
            if code:
                cursor.execute("""
    ...
    # ============ 持仓操作 ============
    def update_position(self, code: str, name: str, shares: int, cost_price: float,
    ...
    # ============ 推荐记录操作 ============
    def add_recommendation(self, code: str, name: str, score: float, 
    ...
    def get_statistics(self) -> Dict:
        ...
        with self.get_cursor() as cursor:
            ...
            """)
            row = cursor.fetchone()
            stats['trades'] = {'count': row['count'], 'total_pnl': row['total_pnl'] or 0}
            ...
            """)
            row = cursor.fetchone()
            stats['positions'] = {'count': row['count'], 'total_value': row['total_value'] or 0}
